...
def getChromeEngine(version):
    ...
    for a in a_list:
        if a.rsplit(".", 1)[0] == version:
            url = url + a + "chromedriver_mac64.zip"
            resp = requests.get(url, headers=settings.HEADERS)
            file_path = os.path.join(settings.DownloadDefaultPath, "chromedriver_mac64.zip")
...
def openChrome():
    file_path = os.path.join(settings.DownloadDefaultPath, "chromedriver_mac64", "chromedriver")
    # 解决版本警告
    s = Service(file_path)
